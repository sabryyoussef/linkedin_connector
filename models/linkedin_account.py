...
class LinkedinAccount(models.Model):
    ...
    oauth_scopes = fields.Char(
        ...
        help=(
            "Space-separated scopes. Recommended: openid profile w_member_social. "
            "openid+profile require the 'Sign In with LinkedIn using OpenID Connect' product in LinkedIn Developers. "
            "w_member_social is for Share on LinkedIn. "
            "With only w_member_social the connector resolves the member via token introspection as a fallback."
        ),
    )
    ...
    def _introspect_member_urn(self):
        ...
        sub = data.get("sub") or data.get("authorized_person") or data.get("member_id")
    ...
    def _fetch_member_urn(self):
        ...
        raise UserError(
            _(
                ...
                "Most likely the access token is expired or the client_id/client_secret are wrong. "
                "Click Disconnect then Connect again.\n\n"
                "If introspect returns 401/403, verify client_id and client_secret are correct.\n"
                "If you want openid+profile, add 'Sign In with LinkedIn using OpenID Connect' in "
                "LinkedIn Developers > Products and reconnect with scopes: openid profile w_member_social."
            ) % {
                "rev": _CONNECTOR_REV,
    ...
    def action_test_post(self):
        ...
        self._post_text("Connected from Odoo LinkedIn Connector. Test post at %s." % ts)
        return {
            "type": "ir.actions.client",
            "tag": "display_notification",
            "params": {
                "title": _("LinkedIn"),
                "message": _("Test post sent successfully."),
                "type": "success",
                "sticky": False,
            },
        }
    ...
    def _post_text(self, text):
        ...
        payload = {
            "author": self.linkedin_member_urn,
        ...
        if response.status_code not in (200, 201):
            raise UserError(_("LinkedIn post failed: %s") % response.text)
